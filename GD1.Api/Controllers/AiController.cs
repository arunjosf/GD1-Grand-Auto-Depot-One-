...
namespace GD1.Api.Controllers
{
    ...
    public class AiController : ControllerBase
    {
            ...
            [FromQuery] string? city)
        {
            ...
            // If no properties found for the specific city, fallback to searching everywhere
            if (!result.Success || result.Data == null || !result.Data.Any())
            {
                result = await _mediator.Send(new GetAllStoragePropertyQuery
                {
                    Latitude = lat,
                    Longitude = lon,
                    City = null,
            ...
            // 2. AI analysis based on property review, compatibility, and distance
            var topLots = result.Data.Take(5).ToList();
            var recommendation = await _gemini.GetBestLotRecommendationAsync(topLots, "Provide the best property considering the physical distance to the user (DistanceKm), available amenities, and user reviews.");
